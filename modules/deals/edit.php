...
$pageTitle = 'Редактировать сделку';
...
if (isAdmin()) {
    $deal = fetchRow("SELECT * FROM deals WHERE id = ?", [$dealId], 'i');
} else {
    $deal = fetchRow("SELECT * FROM deals WHERE id = ? AND manager_id = ?", [$dealId, $userId], 'ii');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Если нет ошибок, обновляем сделку
    if (empty($errors)) {
        if (isAdmin()) {
            $query = "UPDATE deals SET client_id = ?, title = ?, amount = ?, description = ?, status = ?, manager_id = ? WHERE id = ?";
            ...
            $query = "UPDATE deals SET client_id = ?, title = ?, amount = ?, description = ?, status = ? WHERE id = ?";
            $description = !empty($formData['description']) ? $formData['description'] : null;
...
<div class="main-content">
    <div class="top-header">
        <div class="header-title">
            <h1><i class="fas fa-edit"></i> Редактировать сделку</h1>
        </div>
    ...
    <div class="content-area">
        <div class="card">
            <div class="card-header">
                <i class="fas fa-edit"></i> Редактирование сделки
            </div>
            <div class="card-body">
                <?php if (!empty($errors)): ?>
                    ...
                    <div class="row">
                                ...
                                <select class="form-select" id="manager_id" name="manager_id" required>
                                    ...
                                    <?php foreach ($managers as $manager): ?>
                                        <option value="<?php echo $manager['id']; ?>"
                                                <?php echo $formData['manager_id'] == $manager['id'] ? 'selected' : ''; ?>>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select" id="status" name="status">
                                <option value="new" <?php echo $formData['status'] == 'new' ? 'selected' : ''; ?>>Новая</option>
